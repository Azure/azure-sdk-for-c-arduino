...
/*
 * Defines a generic MQTT message to be exchanged between the AzureIoT.h layer
 * and the user application. It uses azure-sdk-for-c's az_span construct, which 
 * is merely a structure that has a pointer to a buffer and its size.
 * Messages without a payload will have the `payload` member set to AZ_SPAN_EMPTY
...
/*
 * Configuration structure passed by `mqtt_client_init_function_t` to the user
 * application for initializing the actual MQTT client.
 */
typedef struct mqtt_client_config_t_struct
{
  /*
   * FQDN address of the broker the MQTT client shall connect to.
   */
  az_span address;

  /*
   * Port of the broker the MQTT client shall connect to.
   */
  int port;

  /*
   * Client ID to be provided in the CONNECT sent by the MQTT client.
   */
  az_span client_id;

  /*
   * Username to be provided in the CONNECT sent by the MQTT client.
   */
  az_span username;

  /*
   * Password to be provided in the CONNECT sent by the MQTT client.
   */
  az_span password;
}
...
/*
 * Generic pointer to the actual instance of the application MQTT client,
 * set by the user application when the `mqtt_client_init_function_t` 
 * calback is invoked.
 */
...
/*
 * Structure that consolidates all the abstracted MQTT functions.
 */
...
/*
 * This function must be provided by the user for the AzureIoT.h layer
