...
static az_result split_jws(az_span jws_span, az_span *header_span,
                           az_span *payload_span, az_span *signature_span) {
  uint8_t *first_dot;
  uint8_t *second_dot;
  ...
  if ((dot_count != 2) ||
      (second_dot >= (az_span_ptr(jws_span) + jws_length - 1))) {
    return AZ_ERROR_UNEXPECTED_CHAR;
  }

  *header_span =
      az_span_create(az_span_ptr(jws_span), first_dot - az_span_ptr(jws_span));
  *payload_span = az_span_create(first_dot + 1, second_dot - first_dot - 1);
  *signature_span = az_span_create(
      second_dot + 1, az_span_ptr(jws_span) + jws_length - second_dot - 1);
 ...
 */
static az_result prvJWS_SHA256Calculate(az_span input_span,
                                        az_span output_span) {
 ...
 * @retval Non-0 if not successful.
 */
static az_result jws_rs256_verify(az_span input_span, az_span signature_span,
                                  az_span n_span, az_span e_span,
                                  az_span buffer_span) {
  az_result result;
  ...
  mbed_tls_result = mbedtls_rsa_import_raw(
      &ctx, az_span_ptr(n_span), az_span_size(n_span), NULL, 0, NULL, 0, NULL,
      0, az_span_ptr(e_span), az_span_size(e_span));

  if (mbed_tls_result != 0) {
    Logger.Error("[JWS] mbedtls_rsa_import_raw res: " +
                 String(mbed_tls_result));
  ...
  if (mbed_tls_result != 0) {
    Logger.Error("[JWS] mbedtls_rsa_check_pubkey res: " +
                 String(mbed_tls_result));
    mbedtls_rsa_free(&ctx);
    return AZ_ERROR_NOT_SUPPORTED;
  }
  ...
  mbed_tls_result = mbedtls_rsa_pkcs1_decrypt(
      &ctx, NULL, NULL, MBEDTLS_RSA_PUBLIC, &decrypted_length,
      az_span_ptr(signature_span), az_span_ptr(buffer_span), jwsRSA3072_SIZE);

  if (mbed_tls_result != 0) {
    Logger.Error("[JWS] mbedtls_rsa_pkcs1_decrypt res: " +
                 String(mbed_tls_result));
    mbedtls_rsa_free(&ctx);
    return AZ_ERROR_NOT_SUPPORTED;
  }
  ...
  result = prvJWS_SHA256Calculate(
      input_span, az_span_create(sha_buffer_ptr, jwsSHA256_SIZE));
  ...
  sha_match_result = memcmp(az_span_ptr(buffer_span) + jwsPKCS7_PAYLOAD_OFFSET,
                            sha_buffer_ptr, jwsSHA256_SIZE);
...
static az_result find_sjwk_value(az_json_reader *payload_json_reader,
                                 az_span *jwk_value_ptr) {
  az_result result = AZ_OK;

  _az_adu_jws_return_if_failed(az_json_reader_next_token(payload_json_reader));

  while (result == AZ_OK) {
    if (az_json_token_is_text_equal(&payload_json_reader->token,
                                    jws_sjwk_json_value)) {
      ...
      _az_adu_jws_return_if_failed(
          az_json_reader_next_token(payload_json_reader));
      _az_adu_jws_return_if_failed(
          az_json_reader_skip_children(payload_json_reader));
  ...
  if (payload_json_reader->token.kind != AZ_JSON_TOKEN_STRING) {
    Logger.Error("[JWS] JSON token type wrong | type: " +
                 String(payload_json_reader->token.kind));
...
static az_result prvFindRootKeyValue(az_json_reader *payload_json_reader,
                                     az_span *kid_span_ptr) {
  az_result result = AZ_OK;
...
static az_result find_key_parts(az_json_reader *payload_json_reader,
  ...
  while (result == AZ_OK && (az_span_size(*base64_encoded_n_span_ptr) == 0 ||
                             az_span_size(*base64_encoded_e_span_ptr) == 0 ||
                             az_span_size(*alg_span_ptr) == 0)) {
    if (az_json_token_is_text_equal(&payload_json_reader->token,
                                    jws_n_json_value)) {
      _az_adu_jws_return_if_failed(
          az_json_reader_next_token(payload_json_reader));
      *base64_encoded_n_span_ptr = payload_json_reader->token.slice;
      ...
      result = az_json_reader_next_token(payload_json_reader);
    } else if (az_json_token_is_text_equal(&payload_json_reader->token,
                                           jws_e_json_value)) {
      _az_adu_jws_return_if_failed(
          az_json_reader_next_token(payload_json_reader));
  ...
  if ((result != AZ_OK) || (az_span_size(*base64_encoded_n_span_ptr) == 0) ||
      (az_span_size(*base64_encoded_e_span_ptr) == 0) ||
      (az_span_size(*alg_span_ptr) == 0)) {
    Logger.Error("[JWS] Parse Signing Key Payload Error: " +
                 String(result, HEX));
    return result;
  }
...
static az_result prvFindManifestSHA(az_json_reader *payload_json_reader,
                                    az_span *sha_span_ptr) {
  az_result result = AZ_OK;
...
static az_result base64_decode_jwk(jws_validation_context *manifest_context) {
  ...
  result = az_base64_decode(manifest_context->jwk_header,
                            manifest_context->jwk_base64_encoded_header,
                            &manifest_context->out_jwk_header_length);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] JWK header az_base64_decode failed: result " +
                 String(result, HEX));

    if (result == AZ_ERROR_NOT_ENOUGH_SPACE) {
      Logger.Error("[JWS] Decode buffer was too small: " +
                   String(jwsJWK_HEADER_SIZE) + " bytes");
    }

    return result;
  }

  manifest_context->jwk_header = az_span_slice(
      manifest_context->jwk_header, 0, manifest_context->out_jwk_header_length);

  result = az_base64_decode(manifest_context->jwk_payload,
                            manifest_context->jwk_base64_encoded_payload,
                            &manifest_context->out_jwk_payload_length);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] JWK payload az_base64_decode failed: result " +
                 String(result, HEX));

    if (result == AZ_ERROR_NOT_ENOUGH_SPACE) {
      Logger.Error("[JWS] Decode buffer was too small: " +
                   String(jwsJWK_PAYLOAD_SIZE) + " bytes");
    }
  ...
  manifest_context->jwk_payload =
      az_span_slice(manifest_context->jwk_payload, 0,
                    manifest_context->out_jwk_payload_length);

  result = az_base64_decode(manifest_context->jwk_signature,
                            manifest_context->jwk_base64_encoded_signature,
                            &manifest_context->out_jwk_signature_length);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] JWK signature az_base64_decode failed: result " +
                 String(result, HEX));

    if (result == AZ_ERROR_NOT_ENOUGH_SPACE) {
      Logger.Error("[JWS] Decode buffer was too small: " +
                   String(jwsSIGNATURE_SIZE) + " bytes");
    }
  ...
  manifest_context->jwk_signature =
      az_span_slice(manifest_context->jwk_signature, 0,
                    manifest_context->out_jwk_signature_length);
...
static az_result
base64_decode_signing_key(jws_validation_context *manifest_context) {
  az_result result;

  result = az_base64_decode(manifest_context->signing_key_n,
                            manifest_context->base64_encoded_n_span,
                            &manifest_context->out_signing_key_n_length);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] Signing key n az_base64_decode failed: result " +
                 String(result, HEX));

    if (result == AZ_ERROR_NOT_ENOUGH_SPACE) {
      Logger.Error("[JWS] Decode buffer was too small: " +
                   String(jwsRSA3072_SIZE) + " bytes");
    }
  ...
  manifest_context->signing_key_n =
      az_span_slice(manifest_context->signing_key_n, 0,
                    manifest_context->out_signing_key_n_length);

  result = az_base64_decode(manifest_context->signing_key_e,
                            manifest_context->base64_encoded_e_span,
                            &manifest_context->out_signing_key_e_length);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] Signing key e az_base64_decode failed: result " +
                 String(result, HEX));

    if (result == AZ_ERROR_NOT_ENOUGH_SPACE) {
      Logger.Error("[JWS] Decode buffer was too small: " +
                   String(jwsSIGNING_KEY_E_SIZE) + " bytes");
    }
  ...
  manifest_context->signing_key_e =
      az_span_slice(manifest_context->signing_key_e, 0,
                    manifest_context->out_signing_key_e_length);
...
static az_result
base64_decode_jws_header_and_payload(jws_validation_context *manifest_context) {
  az_result result;

  result = az_base64_decode(manifest_context->jws_payload,
                            manifest_context->base64_encoded_payload,
                            &manifest_context->out_jws_payload_length);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] JWS payload az_base64_decode failed: result " +
                 String(result, HEX));

    if (result == AZ_ERROR_NOT_ENOUGH_SPACE) {
      Logger.Error("[JWS] Decode buffer was too small: " +
                   String(jwsJWS_PAYLOAD_SIZE) + " bytes");
    }
  ...
  manifest_context->jws_payload =
      az_span_slice(manifest_context->jws_payload, 0,
                    manifest_context->out_jws_payload_length);

  result = az_base64_decode(manifest_context->jws_signature,
                            manifest_context->base64_encoded_signature,
                            &manifest_context->out_jws_signature_length);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] JWS signature az_base64_decode failed: result " +
                 String(result, HEX));
  ...
  manifest_context->jws_signature =
      az_span_slice(manifest_context->jws_signature, 0,
                    manifest_context->out_jws_signature_length);
...
static az_result validate_root_key(jws_validation_context *manifest_context) {
  ...
  result =
      az_json_reader_init(&json_reader, manifest_context->jwk_header, NULL);
  if (az_result_failed(result)) {
    Logger.Error("[JWS] az_json_reader_init failed: result " +
                 String(result, HEX));
  ...
  if (!az_span_is_content_equal(
          az_span_create((uint8_t *)azure_iot_adu_root_key_id,
                         sizeof(azure_iot_adu_root_key_id) - 1),
...
static az_result verify_sha_match(jws_validation_context *manifest_context,
                                  az_span manifest_span) {
  az_json_reader json_reader;
  az_result verification_result;
  az_result result;

  verification_result = prvJWS_SHA256Calculate(
      manifest_span, manifest_context->manifest_sha_calculation);
  ...
  result =
      az_json_reader_init(&json_reader, manifest_context->jws_payload, NULL);
  if (az_result_failed(result)) {
    Logger.Error("[JWS] az_json_reader_init failed: result " +
                 String(result, HEX));
  ...
  if (prvFindManifestSHA(&json_reader, &manifest_context->sha256_span) !=
      AZ_OK) {
    Logger.Error("Error finding manifest signature SHA");
    return AZ_ERROR_ITEM_NOT_FOUND;
  }

  result = az_base64_decode(manifest_context->parsed_manifest_sha,
                            manifest_context->sha256_span,
                            &manifest_context->out_parsed_manifest_sha_size);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] Parsed manifest SHA az_base64_decode failed: result " +
                 String(result, HEX));

    if (result == AZ_ERROR_NOT_ENOUGH_SPACE) {
      Logger.Error("[JWS] Decode buffer was too small: " +
                   String(jwsSHA256_SIZE) + " bytes");
    }
  ...
  manifest_context->parsed_manifest_sha =
      az_span_slice(manifest_context->parsed_manifest_sha, 0,
                    manifest_context->out_parsed_manifest_sha_size);
  ...
  int32_t lComparisonResult = memcmp(
      ...
      az_span_ptr(manifest_context->parsed_manifest_sha), jwsSHA256_SIZE);
...
az_result SampleJWS::ManifestAuthenticate(az_span manifest_span,
                                          az_span jws_span,
  ...
  result = split_jws(jws_span, &manifest_context.base64_encoded_header,
                     &manifest_context.base64_encoded_payload,
                     &manifest_context.base64_encoded_signature);
  ...
  manifest_context.jws_header =
      az_span_create(persistent_scratch_space_head, jwsJWS_HEADER_SIZE);
  persistent_scratch_space_head += jwsJWS_HEADER_SIZE;
  result = az_base64_decode(manifest_context.jws_header,
                            manifest_context.base64_encoded_header,
                            &manifest_context.out_jws_header_length);

  if (az_result_failed(result)) {
    Logger.Error("[JWS] JWS header az_base64_decode failed: result " +
                 String(result, HEX));

    if (result == AZ_ERROR_NOT_ENOUGH_SPACE) {
      Logger.Error("[JWS] Decode buffer was too small: " +
                   String(jwsJWS_HEADER_SIZE) + " bytes");
    }

    return result;
  }

  manifest_context.jws_header = az_span_slice(
      manifest_context.jws_header, 0, manifest_context.out_jws_header_length);
  ...
  result = split_jws(manifest_context.jwk_manifest_span,
                     &manifest_context.jwk_base64_encoded_header,
                     &manifest_context.jwk_base64_encoded_payload,
                     &manifest_context.jwk_base64_encoded_signature);
  ...
  result =
      az_json_reader_init(&json_reader, manifest_context.jwk_payload, NULL);
  if (az_result_failed(result)) {
    Logger.Error("[JWS] az_json_reader_init failed: result " +
                 String(result, HEX));
    return result;
  ...
  if (find_key_parts(&json_reader, &manifest_context.base64_encoded_n_span,
                     &manifest_context.base64_encoded_e_span,
                     &manifest_context.alg_span) != AZ_OK) {
  ...
  manifest_context.scratch_calculation_buffer = az_span_create(
  ...
  result = jws_rs256_verify(
      ...
      manifest_context.jwk_signature,
      az_span_create((uint8_t *)azure_iot_adu_root_key_n,
                     sizeof(azure_iot_adu_root_key_n)),
      az_span_create((uint8_t *)azure_iot_adu_root_key_e,
                     sizeof(azure_iot_adu_root_key_e)),
  ...
  manifest_context.jws_payload =
      az_span_create(reusable_scratch_space_head, jwsJWS_PAYLOAD_SIZE);
  ...
  manifest_context.signing_key_n =
      az_span_create(reusable_scratch_space_head, jwsRSA3072_SIZE);
  ...
  result = jws_rs256_verify(
      az_span_create(az_span_ptr(manifest_context.base64_encoded_header),
                     az_span_size(manifest_context.base64_encoded_header) +
                         az_span_size(manifest_context.base64_encoded_payload) +
                         1),
      manifest_context.jws_signature, manifest_context.signing_key_n,
      manifest_context.signing_key_e,
      manifest_context.scratch_calculation_buffer);
