...
/* split_jws takes a JWS payload and returns pointers to its constituent header, payload, and signature parts. */
static az_result split_jws( az_span jws_span,
                            az_span * header_span,
                            ...
                            az_span * signature_span )
{
    ...
    *signature_span = az_span_create(second_dot + 1, az_span_ptr(jws_span) + jws_length - second_dot - 1);
 ...
 * @retval Non-0 if not successful.
 */
static az_result jws_rs256_verify( az_span input_span,
                                    az_span signature_span,
                                    az_span n_span,
                                    az_span e_span,
                                    az_span buffer_span )
{
    ...
    mbed_tls_result = mbedtls_rsa_import_raw( &ctx,
                                             az_span_ptr(n_span), az_span_size(n_span),
                                             NULL, 0,
                                             NULL, 0,
                                             NULL, 0,
                                             az_span_ptr(e_span), az_span_size(e_span) );
    ...
    /* RSA */
    mbed_tls_result = mbedtls_rsa_pkcs1_decrypt( &ctx, NULL, NULL, MBEDTLS_RSA_PUBLIC, &decrypted_length, az_span_ptr(signature_span), az_span_ptr(buffer_span), jwsRSA3072_SIZE );
    ...
    result = prvJWS_SHA256Calculate( input_span,
    ...
    sha_match_result = memcmp( az_span_ptr(buffer_span) + jwsPKCS7_PAYLOAD_OFFSET, sha_buffer_ptr, jwsSHA256_SIZE );
...
static az_result base64_decode_jwk( jws_validation_context * manifest_context )
{
    ...
    manifest_context->jwk_header = az_span_slice(manifest_context->jwk_header, 0, manifest_context->out_jwk_header_length);
    ...
    manifest_context->jwk_payload = az_span_slice(manifest_context->jwk_payload, 0, manifest_context->out_jwk_payload_length);
    ...
    manifest_context->jwk_signature = az_span_slice(manifest_context->jwk_signature, 0, manifest_context->out_jwk_signature_length);
...
static az_result base64_decode_signing_key( jws_validation_context * manifest_context )
{
    ...
    manifest_context->signing_key_n = az_span_slice(manifest_context->signing_key_n, 0, manifest_context->out_signing_key_n_length);
    ...
    if( az_result_failed( result ) )
    {
        ...
        if( result == AZ_ERROR_NOT_ENOUGH_SPACE )
        {
            Logger.Error( "[JWS] Decode buffer was too small: " + String(jwsSIGNING_KEY_E_SIZE) + " bytes" );
        }

        return result;
    }

    manifest_context->signing_key_e = az_span_slice(manifest_context->signing_key_e, 0, manifest_context->out_signing_key_e_length);
...
static az_result base64_decode_jws_header_and_payload( jws_validation_context * manifest_context )
{
    ...
    manifest_context->jws_payload = az_span_slice(manifest_context->jws_payload, 0, manifest_context->out_jws_payload_length);
    ...
    manifest_context->jws_signature = az_span_slice(manifest_context->jws_signature, 0, manifest_context->out_jws_signature_length);
...
static az_result validate_root_key( jws_validation_context * manifest_context )
{
    ...
    if( !az_span_is_content_equal( az_span_create( ( uint8_t * ) azure_iot_adu_root_key_id,
                                           ...
                                           az_span manifest_span )
{
    ...
    verification_result = prvJWS_SHA256Calculate( manifest_span,
    ...
    if( az_result_failed( result ) )
    {
        Logger.Error( "[JWS] Parsed manifest SHA az_base64_decode failed: result " + String(result, HEX) );
    ...
    manifest_context->parsed_manifest_sha = az_span_slice(manifest_context->parsed_manifest_sha, 0, manifest_context->out_parsed_manifest_sha_size);

    if( manifest_context->out_parsed_manifest_sha_size != jwsSHA256_SIZE )
    {
        Logger.Error( "[JWS] Base64 decoded SHA256 is not the correct length | expected: " +
            String(jwsSHA256_SIZE) + " | actual: " + String(manifest_context->out_parsed_manifest_sha_size) );
        return AZ_ERROR_ITEM_NOT_FOUND;
    }

    int32_t lComparisonResult = memcmp( az_span_ptr(manifest_context->manifest_sha_calculation),
                                           ...
                                           az_span scratch_buffer_span )
{
    ...
    uint8_t * reusable_scratch_space_root = persistent_scratch_space_head + jwsJWS_HEADER_SIZE + jwsJWK_PAYLOAD_SIZE;
    uint8_t * reusable_scratch_space_head = reusable_scratch_space_root;

    /*------------------- Parse and Decode the JWS Header ------------------------*/

    result = split_jws( jws_span,
    ...
    manifest_context.jws_header = az_span_slice(manifest_context.jws_header, 0, manifest_context.out_jws_header_length);
    ...
    if( find_key_parts( &json_reader, &manifest_context.base64_encoded_n_span, &manifest_context.base64_encoded_e_span, &manifest_context.alg_span ) != AZ_OK )
    {
    ...
    manifest_context.scratch_calculation_buffer = az_span_create(reusable_scratch_space_head, jwsSHA_CALCULATION_SCRATCH_SIZE);
    reusable_scratch_space_head += jwsSHA_CALCULATION_SCRATCH_SIZE;
    result = jws_rs256_verify( az_span_create(az_span_ptr(manifest_context.jwk_base64_encoded_header),
                                      az_span_size(manifest_context.jwk_base64_encoded_header) + az_span_size(manifest_context.jwk_base64_encoded_payload) + 1),
    ...
    manifest_context.signing_key_e = az_span_create(reusable_scratch_space_head, jwsSIGNING_KEY_E_SIZE);
    ...
    result = jws_rs256_verify( az_span_create(az_span_ptr(manifest_context.base64_encoded_header),
                                      az_span_size(manifest_context.base64_encoded_header) + az_span_size(manifest_context.base64_encoded_payload) + 1),
    ...
    manifest_context.manifest_sha_calculation = az_span_create(reusable_scratch_space_head,jwsSHA256_SIZE);
    reusable_scratch_space_head += jwsSHA256_SIZE;
    manifest_context.parsed_manifest_sha = az_span_create(reusable_scratch_space_head, jwsSHA256_SIZE);
    reusable_scratch_space_head += jwsSHA256_SIZE;
